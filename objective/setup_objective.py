...
from sampling.MD import run_as_subprocess as run_sampling
...
class setup:
    ...
    def __init__(self,
        ...
        # where the template folder (with input scripts
        # for running sampling methods)
    ...
    def print_initialization_properties_matching(self,
        ...
        self.objective_logger.info(29*"-" + " Initialize %s matching "
                                   % matching_type + 29*"-" + "\n\n")
        self.objective_logger.info("The sub_folder name: %s\n" % sub_folder)

        self.objective_logger.info("The weight of objective "
                                   "function : %.3f \n" % weight)

        self.objective_logger.info("Number of cores for "
                                   "running sampling: %d \n"
                                   % cores_for_sampling)

        self.objective_logger.info("Number of cores for "
                                   "computing objective: %d\n"
                                   % cores_for_objective)

        self.objective_logger.info("The other arbitrary "
                                   "argument: %s \n" % argument)
    ...
    def go_through_each_line(self):
        ...
        if (counter_matching == 0):
            ...
            self.objective_logger.error("ERROR: missing matching type "
                                        "argument or check its format")

            sys.exit("Check errors in the log file")

        if (counter_command == 0):

            self.objective_logger.error("ERROR: missing command to run "
                                        "sampling ( MD or MC ) in the "
                                        "input file ")
    ...
    def This_is_line_is_output_freq(self, each_line):
        ...
        if (len(each_line) == 2 and
                IO.check_type.is_int(each_line[0]) and
                IO.check_type.is_int(each_line[1])):
    ...
    def This_line_is_units(self, each_line, first_line):
        ...
        if (len(each_line) == 1 and
                IO.check_type.is_string(each_line[0])):
    ...
    def This_line_is_matching_type(self, each_line):
        ...
        elif (line_type[1] is False and np.sum(line_type) == 4):

            self.objective_logger.warning("WARRANING:The second argument "
                                          "for objective instances must "
                                          "be string ( subfolder_name )")
    ...
    def check_command(self):
        ...
        if (len(self.command_lst) > 1):
            ...
            self.objective_logger.error("ERROR: currently, only 1 command"
...
# ----------------------------------------------------------------------------
    ...
    def dectect_duplicated_folder(self, type_subfolder):
        ...
        if (len(self.matching_type) > len(set(type_subfolder))):
            ...
            self.objective_logger.error("ERROR: found repeated objective "
                                        "type and its subfolder "
                                        "in input file: !")
    ...
    def setup_working_folders(self, overwrite):
        ...
        # avoid overwriting current working folder
    ...
    def transfer_prepsystem_to_working_folders(self,
                                               matching_type,
                                               subfolder):
    ...
    def generate_objective_dict(self, total_cores):
        ...
        for matching_arg in self.matching_type:
            ...
            matching_argument = " ".join(str(para) for
                                         para in matching_arg[5:])

            (matching_type,
             sub_folder,
             weight,
             cores_for_sampling,
             cores_for_obj) = self.get_objective_input(matching_arg)
            ...
            self.transfer_prepsystem_to_working_folders(matching_type,
                                                        sub_folder)

            ref_address = os.path.join(self.reference_folder_address,
                                       matching_type,
                                       sub_folder)

            IO.check_dir.decide_folder_status(ref_address, "Reference data")

            predict_address = os.path.join(self.predicted_folder,
                                           matching_type,
                                           sub_folder)

            # get folder paths of predicted folder
            (num_predict_folders,
             predict_sub_folders) = self.Get_Path(predict_address)
            ...
            self.sampling_method = run_sampling(self.packagename,
                                                matching_type,
                                                predict_sub_folders,
                                                num_predict_folders,
                                                self.command_lst[0],
            ...
            self.ref_address_dict[matching_type] = {
                                                    sub_folder: ref_sub_folders
                                                    }

            self.predict_address_dict[matching_type] = {
                                                        sub_folder: predict_sub_folders
                                                        }
        ...
        self.check_all_cores(total_cores,
                             total_cores_by_sampling,
                             total_cores_by_obj)
